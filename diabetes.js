...
async function loadModel() {
    try {
        ...
        model = await tf.loadLayersModel('model11/model.json', { strict: false });
...
function validateInputs() {
    let inputData = [
        ...
        parseInt(document.getElementById("BloodPressure").value),
        parseInt(document.getElementById("SkinThickness").value),
...
async function predictHeartDisease(inputData) {
    ...
    if (inputData.length !== 8) {
        console.log("Input data should have exactly 8 features!");
        return;
    }
    ...
    try {
        ...
        resultText.innerText = `Diabetes: ${heartDiseasePercentage}% | No Diabetes: ${noHeartDiseasePercentage}% `+feedbackDisease;
